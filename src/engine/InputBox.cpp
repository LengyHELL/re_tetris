...
InputBox::InputBox(const std::string& style, const Rect& body,
  ...
  text_color(text_color) {
    padding = (int)(cut_size.len() * 1.5);
  }

void InputBox::draw(const Engine& engine) const {
...
void InputBox::update(Engine& engine) {
  ...
  window_size = body.w - 2 * padding;
  t_rect = engine.size_text(text, text_size);
  ...
  cursor_pos = c_rect.w;
  selection_pos = s_rect.w;
  selection_size = abs(c_rect.w - engine.size_text(text.substr(0, engine.get_input_selection()), text_size).w);
